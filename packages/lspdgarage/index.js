let impound = mp.colshapes.newSphere(412.5675, -1023.167, 29.475, 3, 0);
mp.events.add("PushE", (player) => {
  if (mp.players.exists(player)) {
    if(impound.isPointWithin(player.position)) {
        if (player.data.faction == "LSPD") {
           player.call("client:lspdgarage:openMainMenu");
           //player.data.mainmenu = true;
        } else {
            player.notify("~r~Du arbeitest nicht bei LSPD");
        }        
    }    
  }
});

mp.events.add("server:lspdgarage:openMenu",(player) => {
    if (mp.players.exists(player)) {
        gm.mysql.handle.query("SELECT model, v.id, numberPlate, firstname, lastname FROM vehicles v LEFT JOIN characters c ON c.id = v.charId WHERE v.impounded = '6'",[],function(err,res) {
            if (err) console.log("Error in Select lspdgarage Vehicles: "+err);
            if (res.length > 0) {
                var i = 1;
                let vehList = [];
                res.forEach(function(veh) {
                    ...
                    if (parseInt(i) == parseInt(res.length)) {
                        if(mp.players.exists(player)) player.call("client:lspdgarage:openMenu", [JSON.stringify(vehList)]);
                    }
                    i++;
                });
            } else {
                player.notify("~r~Keine Fahrzeuge in der LSPD Garage");
            }
        });
    }    
});
...
mp.events.add("server:lspdgarage:parkout",(player,id) => {
    const one = new mp.Vector3(427.54, -1027.449, 28.985);
    const onehead = -180;
    gm.mysql.handle.query("SELECT * FROM vehicles WHERE id = ?",[id],function(err1,res1) {
        if (err1) console.log("Error in Select Vehicles on lspdgarage: "+err1);
...
mp.events.add("server:lspdgarage:parkin", (player,id) => {
    const pos = new mp.Vector3(427.54, -1027.4492, 28.9851);
