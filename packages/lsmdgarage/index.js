  ...
  if (mp.players.exists(player)) {
    if(impound.isPointWithin(player.position)) {
        if (player.data.faction == "Medic") {
           player.call("client:lsmdgarage:openMainMenu");
           //player.data.mainmenu = true;
        } else {
            player.notify("~r~Du arbeitest nicht bei Medic");
        }        
    }    
  }
});

mp.events.add("server:lsmdgarage:openMenu",(player) => {
    if (mp.players.exists(player)) {
        gm.mysql.handle.query("SELECT model, v.id, numberPlate, firstname, lastname FROM vehicles v LEFT JOIN characters c ON c.id = v.charId WHERE v.impounded = '2'",[],function(err,res) {
            if (err) console.log("Error in Select lsmdgarage Vehicles: "+err);
            if (res.length > 0) {
                var i = 1;
                let vehList = [];
                res.forEach(function(veh) {
                    ...
                    if (parseInt(i) == parseInt(res.length)) {
                        if(mp.players.exists(player)) player.call("client:lsmdgarage:openMenu", [JSON.stringify(vehList)]);
                    }
                    i++;
                });
            } else {
                player.notify("~r~Keine Fahrzeuge in der Medic Garage");
            }
        });
    }    
});
