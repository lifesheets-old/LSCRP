let pc = mp.colshapes.newSphere(-27.654624938964844,-1104.081787109375,26.422351837158203, 1, 0);
let chief = mp.colshapes.newSphere(-31.86248016357422,-1113.7772216796875,26.42235565185547, 1, 0);
...
mp.events.add("inputValueShop", (player, trigger, output, text) => {
            ...
            gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [currentTarget.data.charId], function (err, res) {
               ...
               gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [player.data.charId], function (err1, res1) {
                  ...
                  if (parseFloat(res[0].amount) >= parseFloat(output)) {
                    if(mp.players.exists(currentTarget)) currentTarget.call("client:pdm:requestrechnung",[player,output,res[0].amount,res1[0].amount]);
  ...
  mp.events.add("server:pdm:payrechnung", (player, cop,amount,accountamount, staatskonto) => {
  ...
  mp.events.add("server:pdm:dontPayrechnung", (player, cop) => {
    if(mp.players.exists(player) && mp.players.exists(cop)) {
