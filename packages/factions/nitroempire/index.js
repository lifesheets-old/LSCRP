...
mp.events.add("PushE", (player) => {
    if (mp.players.exists(player)) {
        if (player.data.faction == "NitroEmpire") {   
             if(pc.isPointWithin(player.position) && player.data.mainmenu == false) {
                    player.call("client:nitroempire:createOfficeComputer",[player.data.factionrang]);
                    ...
                    player.call("client:nitroempire:clothes");
                    player.data.mainmenu = true
            }
        }      
    }
  ...
  mp.events.add("server:nitroempire:clothes", (player,name) => {
...
mp.events.add("server:nitroempire:mainMenu", (player,slot) => {
	...
	{
        if (mp.players.exists(player)) {
            player.call("client:nitroempire:closeMainMenu");
		    player.call("client:nitroempire:openMemberMenu");
        }       
    }
    else if (slot == "Dispatches") {
        if(mp.players.exists(player)) {
            gm.mysql.handle.query("SELECT * FROM faction_dispatches WHERE faction = 'NitroEmpire' AND active = 'Y'", [], function(err, res) {
                  ...
                  if (res.length > 0) {
                      ...
                      res.forEach(function(dis) {
                          ...
                          if (parseInt(i) == parseInt(res.length)) {
                              if(mp.players.exists(player)) player.call("client:nitroempire:dispatchvewer", [JSON.stringify(DisList)]);
                          }
                          i++;
                      });
                  } else {
                      if(mp.players.exists(player)) player.call("client:nitroempire:dispatchvewer", ["none"]);
                  }
              });
          }
    }   
...
mp.events.add("server:nitroempire:mark", (player, id) => {
    if (mp.players.exists(player)) {
        gm.mysql.handle.query("SELECT posx,posy FROM faction_dispatches WHERE id = ?", [id], function(err,res) {
            ...
            player.call("client:nitroempire:markdispatch", [res[0].posx, res[0].posy])
          });
    }    
  });
  
  mp.events.add("server:nitroempire:deletedispatch", (player, id) => {
            ...
            if (currentTarget.data.faction == "NitroEmpire") {
                if(currentTarget !== null) {        
                    ...
                    gm.mysql.handle.query("UPDATE characters SET faction = ?, duty = ?, factionrang = ?, factioncloth = 'Zivil' WHERE id = ?", [currentTarget.data.faction, currentTarget.data.factionDuty,currentTarget.data.factionrang,currentTarget.data.charId], function(err12, ress12) {
                        if(err12) console.log("Error in NitroEmpire Dismiss Member");
                    });
        ...
        if (mp.players.exists(currentTarget)) {
            if (currentTarget !== null) {
                if (currentTarget.data.faction == "NitroEmpire") {
                    ...
                    player.notify("~r~Die Person ist nicht im NitroEmpire");
                }
            } else {
                player.notify("~r~Keiner in deiner Nähe");
            }
    ...
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'NitroEmpire'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(acls) {
                ...
                ACLSList.push(obj);
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:nitroempire:Memberlist", [JSON.stringify(ACLSList)]);
                }
                i++;
            });
        } else {   
            if (mp.players.exists(player)) player.notify("Es gibt keine NitroEmpire Beamten");
        }
     });
});

mp.events.add("server:nitroempire:mitarbeiter",(player,id) => {
    if (mp.players.exists(player)) {
...
mp.events.add("server:nitroempire:spawnVehicle",(player,type) => {
            ...
            if (getVehicleFromPosition(three, 3).length > 0) {     
                ...
                if (mp.players.exists(player)) {
                    if (mp.vehicles.exists(veh)) {
                        ...
                        veh.setVariable("faction", "nitroempire");
    ...
    if (mp.players.exists(player)) {
        ...
        if (mp.players.exists(currentTarget)) {
            if (currentTarget.data.faction == "Civillian") {
                gm.mysql.handle.query("UPDATE characters SET faction = 'NitroEmpire', factionrang = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                    if (err) console.log("Error in Update Faction user: "+err);
                    player.notify("~g~Der Bürger wurde eingestellt");
                    currentTarget.notify("Sie wurden beim NitroEmpire eingestellt!");
                    currentTarget.data.faction = "NitroEmpire";
                    currentTarget.data.factionDuty = 0;
                    currentTarget.data.factionrang = 1;
                    currentTarget.data.factiondn = 0;
                    currentTarget.setVariable("faction","NitroEmpire");
...
mp.events.add("server:nitroempire:parkin",(player,x,y,z) => {
    if (mp.players.exists(player)) {
...
mp.events.add("inputValueShop", (player, trigger, output, text) => {
            ...
            gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [currentTarget.data.charId], function (err, res) {
               ...
               gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [player.data.charId], function (err1, res1) {
                  ...
                  if (parseFloat(res[0].amount) >= parseFloat(output)) {
                    if(mp.players.exists(currentTarget)) currentTarget.call("client:nitroempire:requestrechnung",[player,output,res[0].amount,res1[0].amount]);
  ...
  mp.events.add("server:nitroempire:payrechnung", (player, cop,amount,accountamount, staatskonto) => {
