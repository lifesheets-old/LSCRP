let pc = mp.colshapes.newSphere(-1189.7681884765625, -1724.8416748046875, 4.3893866539001465, 2, 0);
let chief = mp.colshapes.newSphere(-1211.6492919921875, -1727.3145751953125, 4.4199981689453125, 2, 0);

mp.events.add("PushE", (player) => {
    if (mp.players.exists(player)) {
        if (player.data.faction == "Exotic cars") {
            if(pc.isPointWithin(player.position) && player.data.mainmenu == false) {
                player.call("client:ex:createOfficeComputer",[player.data.factionrang]);  
                player.data.mainmenu = true;
            } 
			if(chief.isPointWithin(player.position) && player.data.mainmenu == false && player.data.factionrang > 1) {
                player.call("client:ex:createChiefMenu",[player.data.factionrang]);
                player.data.mainmenu = true;
            }
        }         
    }
...
mp.events.add("server:ex:mainMenu", (player,slot,name) => {
    getNearestPlayer(player, 2);   
    if (name == "Mitarbeiterverwaltung")
    {
        player.call("client:ex:openMemberMenu");
    } 
});
mp.events.add("server:ex:entlassen", (player,itemText) => {
    getNearestPlayer(player, 2);   
    if(currentTarget !== null) {    
        if (currentTarget.data.faction == "Exotic cars") {
            currentTarget.notifyWithPicture("Exotic cars", "Entlassung", "Deine Anstellung wurde beendet","CHAR_CALL911");
        currentTarget.call("client:ex:delmarkers");
        player.notifyWithPicture("Exotic cars", "Mitarbeiterverwaltung","Du hast "+currentTarget.data.firstname + " "+ currentTarget.data.lastname + " entlassen.","CHAR_CALL911");
        ...
        gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
            ...
            player.notify("~r~Der Spieler arbeitet nicht bei der Exotic cars!")
        } 
    }   
});

mp.events.add("server:ex:befördern", (player,rank) => {
    if (mp.players.exists(player)) {
        getNearestPlayer(player,2);
        if (currentTarget !== null) {
            if (currentTarget.data.faction == "Exotic cars") {
                ...
                player.notify("~r~Die Person ist nicht bei Exotic cars");
            }
        } else {
...
mp.events.add("server:ex:mitarbeiter",(player) => {
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'Exotic cars'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(justiz) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:ex:Memberlist", [JSON.stringify(LSCList)]);
                }
                i++;
            });
        } else {   
            player.notify("Es gibt keine Exotic cars Mitarbeiter");
        }
     });
});

mp.events.add("server:ex:mitarbeiterentl",(player,id) => {
...
mp.events.add("server:ex:einstellen",(player) => {
    getNearestPlayer(player, 2);  
    if (currentTarget !== null) {
        if (currentTarget.data.faction == "Civillian") {
            gm.mysql.handle.query("UPDATE characters SET faction = 'Exotic cars', factionrang = '1',firma = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                if (err) console.log("Error in Update Faction user: "+err);
                player.notify("~g~Der Bürger wurde eingestellt");
                currentTarget.notify("Sie wurden bei Exotic cars eingestellt!");
                currentTarget.data.faction = "Exotic cars";
                currentTarget.data.factionDuty = 0;
...
mp.events.add("inputValueShop", (player, trigger, output, text) => {
            ...
            gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [currentTarget.data.charId], function (err, res) {
               ...
               gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [player.data.charId], function (err1, res1) {
                  ...
                  if (parseFloat(res[0].amount) >= parseFloat(output)) {
                    if(mp.players.exists(currentTarget)) currentTarget.call("client:ex:requestrechnung",[player,output,res[0].amount,res1[0].amount]);
  ...
  mp.events.add("server:ex:payrechnung", (player, cop,amount,accountamount, staatskonto) => {
  ...
  mp.events.add("server:ex:dontPayrechnung", (player, cop) => {
    if(mp.players.exists(player) && mp.players.exists(cop)) {
