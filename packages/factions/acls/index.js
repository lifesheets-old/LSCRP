let duty = mp.colshapes.newSphere(948.23, -966.28, 39.49, 2, 0); //ckeck
let clothes = mp.colshapes.newSphere(956.79, -966.57, 39.50, 2, 0); //Klamotten
// let vehicle = mp.colshapes.newSphere(936, -987.13, 38.2824, 3, 0); //check            Fahrzeugspawn vorerst deaktiviert
let pc = mp.colshapes.newSphere(951.28, -968.29, 39.50, 2, 0); //check
// let parkin = mp.colshapes.newSphere(944, -947, 39.49, 3, 0); //check                  Firmenfahrzeugeinparkpunkt deaktiviert
let chief = mp.colshapes.newSphere(0,0,-500, 2, 0); 
                            ...
                            if(currentTarget)
                            {
                                var vehicles = getVehicleFromPosition(player.position, 3);
                                    if (vehicles.length > 0)
                                    {
                                        ...
                                        {
                                            vehicles[0].repair();
                                            vehicles[0].setVariable("HP", 1000);
...
mp.events.add("inputValueShop", (player, trigger, output, text) => {
      ...
      if(trigger === "aclsRechnung") {
        ...
        if (mp.players.exists(currentTarget)) {
         gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE ownerId = ?", [currentTarget.data.charId], function (err, res) {
            if (err) console.log("Error in get Player bank amount at acls rechnung: "+err);
           gm.mysql.handle.query("SELECT amount FROM `bank_konten` WHERE kontonummer = ?", [9876543210], function (err1, res1) {
              ...
              if (res.length > 0) {                
                  if (parseFloat(res[0].amount) >= parseFloat(output)) {
                    if(mp.players.exists(currentTarget)) currentTarget.call("client:acls:requestrechnung",[player,output,res[0].amount,res1[0].amount]);
  ...
  mp.events.add("server:acls:payrechnung", (player, cop,amount,accountamount, staatskonto) => {
    if(mp.players.exists(player) && mp.players.exists(cop)) {
      ...
      gm.mysql.handle.query("UPDATE `bank_konten` SET amount = ? WHERE ownerId = ?",[newamount,player.data.charId], function(err, res) {
        if (err) {
          console.log("Error in Pay acls rechnung Query: "+err);
          ...
          gm.mysql.handle.query("UPDATE bank_konten SET amount = ? WHERE kontonummer = ?", [staatamout, 9876543210], function(err1, res1) {
            ...
            var bs_description = "acls rechnung ("+player.data.firstname+" "+player.data.lastname+")";
  ...
  mp.events.add("server:acls:dontPayrechnung", (player, cop) => {
    if(mp.players.exists(player) && mp.players.exists(cop)) {
