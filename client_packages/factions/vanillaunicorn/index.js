...
mp.events.add("client:vu:openMainMenu", (factionrang) => {
  mainMenujustiz = new Menu("Unternehmung","",MenuPoint);  
  ...
  mainMenujustiz.ItemSelect.on((item, index) => {
    if (item.Text !== "Rechnung ausstellen") {
      ...
      mp.events.call("createInputShop", "vuRechnung");
      mainMenujustiz.Close();
    }    
  });
...
mp.events.add("sendInputrechnung",(trigger,output) => {
  ...
  mp.events.add("client:vu:requestrechnung", (cop, amount, accountamount, factionkonto) => {
    ...
    ui_ticket.ItemSelect.on((item, index, value) => {
        if (item.Text == 'Bezahlen') {
            mp.events.callRemote("server:vu:payrechnung",cop,amount,accountamount, factionkonto);
            ui_ticket.Close();
        } else if (item.Text == 'Ablehnen') {
            mp.events.callRemote("server:vu:dontPayrechnung",cop);
            ui_ticket.Close();
        }
    });
  });
