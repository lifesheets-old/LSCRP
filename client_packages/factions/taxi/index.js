...
mp.events.add("client:taxi:openMainMenu", (factionrang) => {
  ...
  mainMenujustiz.ItemSelect.on((item, index) => {
    if (item.Text !== "Rechnung ausstellen") {
      ...
      mp.events.call("createInputShop", "taxiRechnung");
      mainMenujustiz.Close();
    }    
  });
...
mp.events.add("sendInputrechnung",(trigger,output) => {
  ...
  mp.events.add("client:taxi:requestrechnung", (cop, amount, accountamount, staatskonto) => {
    ...
    ui_ticket.ItemSelect.on((item, index, value) => {
        if (item.Text == 'Bezahlen') {
            mp.events.callRemote("server:taxi:payrechnung",cop,amount,accountamount, staatskonto);
            ui_ticket.Close();
        } else if (item.Text == 'Ablehnen') {
            mp.events.callRemote("server:taxi:dontPayrechnung",cop);
            ui_ticket.Close();
        }
    });
  });

mp.events.add("client:taxi:createChiefMenu",() => {
...
mp.events.add("client:taxi:createOfficeComputer", (factionrang) => {
  const officeComputer = new Menu("Downtown Cab. Co","Computer",MenuPoint);
 officeComputer.AddItem(new UIMenuItem("Leitstelle","Leitstelle übernehmen/abgeben Nummer: 555"));
 if (factionrang > 1) {
 ...
 officeComputer.ItemSelect.on((item, index) => {
   if (item.Text == "Leitstelle") {
     mp.events.callRemote("server:phone:getLeitstelle",555);
