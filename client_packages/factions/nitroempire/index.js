...
mp.events.add("client:nitroempire:openMainMenu", (factionrang) => {
  mainMenuACLS = new Menu("NitroEmpire","",MenuPoint);
  ...
  mainMenuACLS.AddItem(new UIMenuItem("Fahrzeug reparieren","Fahrzeug reparieren"));
    ...
    mainMenuACLS.ItemSelect.on((item, index) => {
      ...
      if (item.Text !== "Rechnung ausstellen") {
        mp.events.callRemote("server:nitroempire:mainMenu",item.Text);
        mainMenuACLS.Close();
      } else {
        mp.events.call("createInputShop", "nitroRechnung");
        mainMenuACLS.Close();
      }          
    });
...
mp.events.add("sendInputrechnung",(trigger,output) => {
  ...
  mp.events.add("client:nitroempire:requestrechnung", (cop, amount, accountamount, staatskonto) => {
    ...
    ui_ticket.ItemSelect.on((item, index, value) => {
        if (item.Text == 'Bezahlen') {
            mp.events.callRemote("server:nitroempire:payrechnung",cop,amount,accountamount, staatskonto);
            ui_ticket.Close();
        } else if (item.Text == 'Ablehnen') {
            mp.events.callRemote("server:nitroempire:dontPayrechnung",cop);
            ui_ticket.Close();
        }
    });
  });
//Leitstelle
mp.events.add("client:nitroempire:createOfficeComputer", (factionrang) => {
  const officeComputer = new Menu("NitroEmpire","Computer",MenuPoint);
 officeComputer.AddItem(new UIMenuItem("Leitstelle","Leitstelle übernehmen/abgeben Nummer: 988"));
 if (factionrang > 1) {
  officeComputer.AddItem(new UIMenuItem("Mitarbeiter","Mitarbeiter vom NitroEmpire"));
 } 
 officeComputer.Visible = true; 

 officeComputer.ItemSelect.on((item, index) => {
   if (item.Text == "Leitstelle") {
     mp.events.callRemote("server:phone:getLeitstelle",988); 
...
mp.events.add("client:nitroempire:openofficeComputer", () => {
...
mp.events.add("client:nitroempire:openMemberMenu", () => {
  memberMenuACLS = new Menu("NitroEmpire","Mitarbeiterverwaltung",MenuPoint);
...
//MemberMenü Ende
//Cothes Menu
clothesACLS = new Menu("NitroEmpire", "Umkleide", MenuPoint);
...
mp.events.add("client:nitroempire:clothes", () => {
  clothesACLS.Visible = true;
});

clothesACLS.ItemSelect.on((item, index) => {      
    mp.events.callRemote("server:nitroempire:clothes",item.Text);
});
...
function drawDipatch(disJSON){
  ...
  ui_Dispatchv.ItemSelect.on((item, index) => {
      mp.events.call("client:nitroempire:dispatchsub", item.Description);
      ui_Dispatchv.Close();
  });
}
mp.events.add("client:nitroempire:dispatchvewer", drawDipatch);

mp.events.add("client:nitroempire:dispatchsub", (position) => {
  ...
  ui_subdispatch.ItemSelect.on((item) => {
      if (item.Text == 'Dispatch Makieren') {
          mp.events.callRemote("server:nitroempire:mark",item.Description)
          ui_subdispatch.Close();
      } else if (item.Text == 'Dispatch löschen') {
          mp.events.callRemote("server:nitroempire:deletedispatch",item.Description);
          ui_subdispatch.Close();
      }
  });
});

mp.events.add("client:nitroempire:markdispatch",(posx, posy) => {
...
mp.events.add("client:nitroempire:askedForDismiss", () => {
  dismissACLS = new Menu("NitroEmpire","Willst du den Mitarbeiter wirklich entlassen?",MenuPoint);
  ...
  dismissACLS.ItemSelect.on((item, index) => {      
    mp.events.callRemote("server:nitroempire:dismissMember",item.Text);
  });
});
mp.events.add("client:nitroempire:closeDismissMenu", () => {
  dismissACLS.Close();
});
...
mp.events.add("client:nitroempire:memberSub",(id) => {
  ...
  memberSub.Visible = true;
    ...
    if (item.Text == "Kündigen") {
      mp.events.callRemote("server:nitroempire:mitarbeiter",id);
      memberSub.Close();
    } else if (item.Text == "Schließen") {
      memberSub.Close();
    }
...
mp.events.add("client:nitroempire:createVeichleMenu", (factionrang) => {
  ...
  vehicles.ItemSelect.on((item, index, value) => {
    if (item.Text !== "Schließen") {
      mp.events.callRemote("server:nitroempire:spawnVehicle",item.Description);
      vehicles.Close();
    } else if (item.Text == "Schließen") {
      vehicles.Close();
    }
...
mp.events.add("client:nitroempire:vehicleblip", (posX, posY, posZ) => {
...
// Dispatch blips
mp.events.add("client:nitroempire:showDispatch", (posX, posY, posZ, id) => {
