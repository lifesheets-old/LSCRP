...
function drawVehicles(vehJSON){
    ...
    ui_List.ItemSelect.on((item, index) => {
        mp.events.callRemote("server:lsmdgarage:parkout", item.Description);
        ui_List.Close();
    });
  }
  mp.events.add("client:lsmdgarage:openMenu", drawVehicles);

  mp.events.add("client:lsmdgarage:openMainMenu",() => {
    const ui_sub = new Menu("Medicgarage", "Park in or Out", MenuPoint);
    ...
    ui_sub.ItemSelect.on((item, index, value) => {
        if (item.Text == "Fahrzeug ausparken") {
          mp.events.callRemote("server:lsmdgarage:openMenu");   
          ui_sub.Close();          
        } else if (item.Text == "Fahrzeug einparken") {
            mp.events.callRemote("server:lsmdgarage:parkin");  
            ui_sub.Close(); 
        } else {            
            ui_sub.Close();
        }      
