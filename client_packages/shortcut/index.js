...
mp.events.add("client:anim:openShortcut", (json) => {
  ...
  mp.events.add("client:shortcut:step2", (id) => {
    ...
    ui_shortsub.AddItem(new UIMenuItem("Sitzen [NUR HUNDE]", ""));
    ui_shortsub.AddItem(new UIMenuItem("Liegen [NUR HUNDE]", ""));
    ...
    ui_shortsub.ItemSelect.on((item, index, value) => {
     if (item.Text == "Sitzen (Männl.)") {
       ...
       ui_shortsub.Close();
      } else if (item.Text == "Sitzen [NUR HUNDE]") {
        let p1 = '"creatures@rottweiler@amb@world_dog_sitting@base"'
        let p2 = '"base"'
        mp.events.callRemote("server:shortcut:save",item.Text, id,p1,p2,1,33);
        ui_shortsub.Close();    
      } else if (item.Text == "Liegen [NUR HUNDE]") {
        let p1 = '"creatures@rottweiler@amb@sleep_in_kennel@"'
        let p2 = '"sleep_in_kennel"'
        mp.events.callRemote("server:shortcut:save",item.Text, id,p1,p2,1,33);
        ui_shortsub.Close(); 
